"""3-month backtest for ETH and GRAM."""
...
def run_bt(df, sig_config, strategy_params, trail_params, risk_config, dynamic_risk, funding, symbol):
    ...
    trades, metrics = run_backtest(
        ...
        breakeven_at=trail_params.get('breakeven_at', 0.3),
        trailing_activate=trail_params.get('trail_activate', 0.8),
        trailing_step=trail_params.get('trail_step', 0.3),
    ...
    print(f"  Avg Win $: {np.mean([t['pnl'] for t in win_trades]):.2f}" if win_trades else "")
    print(f"  Avg Loss $: {np.mean([t['pnl'] for t in lose_trades]):.2f}" if lose_trades else "")
    print(f"  Max Consec Losses: {m.get('max_consec_losses', '?')}")
    print(f"  Total Funding: ${m.get('total_funding', 0):.2f}")

    # Trade list
    if trades:
        print(f"\n  #  | {'Date':>10} | {'Side':>4} | Entry    | Exit     | PnL      | Reason")
        print(f"  {'-'*75}")
        for i, t in enumerate(trades):
            ...
            print(f"  {i+1:>3} | {ts:>10} | {side:>4} | {entry:>8.2f} | {exit_p:>8.2f} | {pnl:>+8.2f} | {reason} {marker}")
...
def main():
    ...
    default_trail = config.get('trailing', {})
    ...
    # ─── ETH ───
    print("=" * 60)
    print("  ETHUSDT — 3 MONTH BACKTEST")
    print("=" * 60)
    ...
    eth_trail = eth_cfg.get('trailing', default_trail)
    eth_filters = eth_cfg.get('filters', {})

    sig_config = {'strategy': eth_strat, 'filters': eth_filters}

    df_eth = fetch_ohlcv(bybit, 'ETH/USDT:USDT', since=three_months_ago_ms)
    print(f"  Candles: {len(df_eth)}, {df_eth['timestamp'].iloc[0]} → {df_eth['timestamp'].iloc[-1]}")
    years = (df_eth['timestamp'].iloc[-1] - df_eth['timestamp'].iloc[0]).days / 365.25
    print(f"  Period: {years:.2f} years ({years*12:.1f} months)")

    funding_eth = fetch_funding_rates(bybit, 'ETH/USDT:USDT')
    print(f"  Funding records: {len(funding_eth)}")

    run_bt(df_eth, sig_config, eth_strat, eth_trail, risk_config, dynamic_risk, funding_eth, 'ETHUSDT')

    # ─── GRAM (merged) ───
    print(f"\n{'=' * 60}")
    print("  GRAMUSDT — 3 MONTH BACKTEST (merged TON+GRAM)")
    print("=" * 60)
    ...
    gram_trail = gram_cfg.get('trailing', default_trail)
    gram_filters = gram_cfg.get('filters', {})

    sig_config_gram = {'strategy': gram_strat, 'filters': gram_filters}

    split_ts = pd.Timestamp(SPLIT_DATE)
    df_ton = fetch_ohlcv(binance, 'TON/USDT', since=three_months_ago_ms)
    df_gram = fetch_ohlcv(bybit, 'GRAM/USDT:USDT', since=three_months_ago_ms)

    print(f"  TON (Binance) before {SPLIT_DATE}: {len(df_ton)}c")
    print(f"  GRAM (Bybit) after {SPLIT_DATE}: {len(df_gram)}c")

    # For3 months, GRAM only existed for ~2.5 months (June 15 - Aug23)
    # Use TON for the rest
    ...
    run_bt(df_gram_merged, sig_config_gram, gram_strat, gram_trail, risk_config, dynamic_risk, funding_gram, 'GRAMUSDT')
