"""Backtest ETH + GRAM on 1D candles — use full available data."""
...
def run_bt(df, sig_config, risk_config, dynamic_risk, funding, symbol):
    ...
    trades, metrics = run_backtest(
        ...
        cooldown_hours=24.0,  # 1D cooldown
    ...
    print(f"  Avg Win $: {np.mean([t['pnl'] for t in win]):.2f}" if win else "")
    print(f"  Avg Loss $: {np.mean([t['pnl'] for t in lose]):.2f}" if lose else "")
...
def main():
    ...
    default_strat['timeout'] = 3  # 3 * 1D = 3 day timeout
    ...
    # ─── ETH on 1D ───
    print("=" * 60)
    print("  ETHUSDT — FULL DATA ON 1D CANDLES")
    print("=" * 60)
    ...
    eth_strat['timeout'] = 3  # 3 * 1D
    eth_filters = eth_cfg.get('filters', {})

    sig_config = {'strategy': eth_strat, 'filters': eth_filters}

    df_eth = fetch_ohlcv(bybit, 'ETH/USDT:USDT', timeframe=INTERVAL)
    ...
    print(f"  Period: {years:.1f} years ({years*12:.0f} months)")
    ...
    # ─── GRAM on 1D ───
    print(f"\n{'=' * 60}")
    print("  GRAMUSDT — FULL DATA ON 1D CANDLES (merged TON+GRAM)")
    print("=" * 60)
    ...
    gram_strat['timeout'] = 3  # 3 * 1D
    gram_filters = gram_cfg.get('filters', {})

    sig_config_gram = {'strategy': gram_strat, 'filters': gram_filters}

    split_ts = pd.Timestamp(SPLIT_DATE)
    df_ton = fetch_ohlcv(binance, 'TON/USDT', timeframe=INTERVAL)
    df_gram = fetch_ohlcv(bybit, 'GRAM/USDT:USDT', timeframe=INTERVAL)
    ...
    years_g = (df_gram_merged['timestamp'].iloc[-1] - df_gram_merged['timestamp'].iloc[0]).days / 365.25
    print(f"  Period: {years_g:.1f} years ({years_g*12:.0f} months)")
