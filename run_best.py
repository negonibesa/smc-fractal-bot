...
loader = BybitLoader()
df_4h = loader.load_from_csv('BNBUSDT_4h_12m.csv')

# Best config from auto-optimizer
params = {
    'lookback': 12,
...
print(f"{'='*60}")
print(f"  BNBUSDT — BACKTEST (Best Config)")
print(f"{'='*60}")
...
save_backtest_report(metrics, trades, 'logs/backtest_BNB_best_config_report.txt')
print(f"\n  Report saved to logs/backtest_BNB_best_config_report.txt")
