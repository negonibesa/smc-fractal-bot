"""Check DOGEUSDT signals (3 months)."""
import sys, os
...
symbol = 'DOGEUSDT'
result = client.get_klines(symbol, interval='240', limit=2200)
...
print(f"DOGEUSDT: {len(df_3m)} candles ({df_3m['timestamp'].min()} to {df_3m['timestamp'].max()})")
...
if signals_3m:
    ...
    for t in trades:
        print(f"  {t['direction']} entry={t['entry_price']:.6f} exit={t['exit_price']:.6f} pnl=${t['pnl']:.2f} ({t['exit_reason']})")
else:
    print("\nNo signals found.")
